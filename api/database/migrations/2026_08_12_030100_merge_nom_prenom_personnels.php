...
{
    /** Même fusion que sur `eleves` (cf. 2026_08_12_030000), pour le personnel. */
    public function up(): void
    {
        ...
        DB::table('personnels')->select('id', 'nom', 'prenom')->orderBy('id')->get()->each(
            fn ($row) => DB::table('personnels')->where('id', $row->id)
    ...
    public function down(): void
    {
        ...
        DB::table('personnels')->select('id', 'nom_complet')->orderBy('id')->get()->each(function ($row) {
            $parties = explode(' ', trim($row->nom_complet), 2);
            DB::table('personnels')->where('id', $row->id)->update([
                'prenom' => $parties[0] ?? '',
                'nom' => $parties[1] ?? '',
            ]);
        });
