...
class FactureVenteGenerator
{
    ...
    public function build(VenteFourniture $vente): string
    {
        ...
        $mpdf->WriteHTML(
            ...
            .'</style></head><body>'
            .$this->enTete($school)
            .'<div class="titre">FACTURE<br>FOURNITURES SCOLAIRES</div>'
            .($vente->estAnnulee() ? '<div class="annule">— VENTE ANNULÉE —</div>' : '')
            .$this->mentions($vente)
            .$this->lignes($vente)
            .$this->pied($vente)
            .'</body></html>'
    ...
    private function mentions(VenteFourniture $vente): string
    {
        ...
        $lignes = [
            ...
            ['Acheteur', $acheteur],
            ['Paiement', $this->libelleMode($vente->mode)],
        ];
    ...
    private function lignes(VenteFourniture $vente): string
    {
        ...
        $html .= '<tr><td><b>Article</b></td><td class="qte"><b>Qté</b></td>'
            .'<td class="pu"><b>P.U.</b></td><td class="montant"><b>Total</b></td></tr>';
    ...
    private function pied(VenteFourniture $vente): string
    {
        return '<div class="sep"></div><div class="pied">'
            .'Vendu par : '.$this->e($vente->vendeur?->name ?? '—').'<br>'
            .($vente->note ? $this->e($vente->note).'<br>' : '')
            .'<i>Article vendu n\'est ni repris ni échangé.</i>'
            .'</div>';
    }
