...
class SeanceController extends Controller
{
    ...
    public function store(Request $request, int $classeId): JsonResponse
    {
        ...
        $data = $this->resoudreSalle($data, $classe->school_id);
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        $data = $this->resoudreSalle($data, $seance->school_id);
        $seance->update($data);
    ...
    private function presenter(Seance $seance, User $user): array
    {
        return [
            ...
            'salle_details' => $seance->salleReference ? ['id' => $seance->salleReference->id, 'nom' => $seance->salleReference->nom] : null,
    ...
    private function resoudreSalle(array $data, int $schoolId): array
    {
        if (! empty($data['salle_id'])) {
            $salle = Salle::forSchool($schoolId)->findOrFail($data['salle_id']);
        ...
        $data['salle'] = $nom === '' ? null : $nom;
        $data['salle_id'] = $nom === '' ? null : Salle::firstOrCreate(
            ['school_id' => $schoolId, 'nom' => $nom],
            ['active' => true],
        )->id;
        return $data;
    }
