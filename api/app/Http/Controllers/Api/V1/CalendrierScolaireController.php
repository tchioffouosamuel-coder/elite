...
class CalendrierScolaireController extends Controller
{
    ...
    {
        return [
            'id' => $r->id, 'date' => Carbon::parse((string) $r->date)->toDateString(), 'est_ouvert' => $r->est_ouvert, 'motif' => $r->motif,
            'classe_id' => $r->classe_id, 'classe' => $r->classe?->nom,
            'niveau_id' => $r->niveau_id, 'niveau' => $r->niveau?->name_fr ?? $r->niveau?->code,
            'sous_systeme_id' => $r->sous_systeme_id, 'sous_systeme' => $r->sousSysteme?->nom,
        ];
    }
}