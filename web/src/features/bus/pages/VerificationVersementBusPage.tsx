...
import { fetchVerificationVersementBus, type VerificationVersementBus } from '@/features/bus/api'
import { francs } from '@/features/finance/api'
import type { ApiError } from '@/shared/types/api'

const LIBELLES_MODE: Record<VerificationVersementBus['mode'], string> = {
  especes: 'Espèces',
...
/**
 * Page publique ouverte en scannant le QR code d'un reçu de transport
 * scolaire — même principe que `VerificationVersementPage` pour la
 * scolarité, mais sur le registre séparé des paiements de bus.
 */
export function VerificationVersementBusPage() {
  const { versementId, signature } = useParams<{ versementId: string; signature: string }>()
  const [versement, setVersement] = useState<VerificationVersementBus | null>(null)
  ...
  useEffect(() => {
    ...
    fetchVerificationVersementBus(Number(versementId), signature)
      .then(setVersement)
        ...
        {!chargement && versement && (
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <div className="mt-2 w-full space-y-2 rounded-xl bg-cream-50 p-4 text-left text-sm">
              ...
              {versement.ecole && <Ligne label="École" valeur={versement.ecole} />}
