...
export async function fetchProgramme(
  ...
  const { data } = await http.get<ApiResponse<Programme>>(
    `/classe-matieres/${classeMatiereId}/progression`,
  );
  return data.data;
}
...
export async function enregistrerProgramme(
  ...
  const { data } = await http.put<ApiResponse<Programme>>(
    `/classe-matieres/${classeMatiereId}/progression`,
    { items },
  );
  return data.data;
...
export async function enregistrerCartouche(
  ...
  Pick<Programme, "departement" | "specialite" | "module_competence">
> {
  const { data } = await http.put<
    ApiResponse<
      Pick<Programme, "departement" | "specialite" | "module_competence">
    >
  >(`/classe-matieres/${classeMatiereId}/progression/cartouche`, payload);
  return data.data;
}
...
export async function fetchProgressionColonnes(
  classeMatiereId: number,
...
export async function importerProgression(
  classeMatiereId: number,
  fichier: File,
 ...
 */
export async function ouvrirFicheProgressionPdf(
  classeMatiereId: number,
): Promise<void> {
  const fenetre = window.open("", "_blank");

  const response = await http.get(
    `/classe-matieres/${classeMatiereId}/progression/pdf`,
    {
      responseType: "blob",
    },
  );
...
export async function fetchProgressionClasse(
  classeId: number,
): Promise<TauxMatiere[]> {
  const { data } = await http.get<ApiResponse<TauxMatiere[]>>(
    `/classes/${classeId}/progression`,
  );
  return data.data;
}
...
export async function telechargerModeleProgressionClasse(
  classeId: number,
  nomClasse: string,
): Promise<void> {
  await telechargerFichier(
    `/classes/${classeId}/progression/modele`,
    undefined,
    `modele-progression-${nomClasse}.xlsx`,
  );
}

export async function importerProgressionClasse(
  classeId: number,
  fichier: File,
): Promise<ResultatImportClasse> {
  const corps = new FormData();
  corps.append("fichier", fichier);

  const { data } = await http.post<ApiResponse<ResultatImportClasse>>(
    `/classes/${classeId}/progression/import`,
    corps,
  );
  return data.data;
}
...
export async function fetchMesAffectations(
  date?: string,
): Promise<AffectationJournee[]> {
  const { data } = await http.get<ApiResponse<AffectationJournee[]>>(
    "/ma-journee",
    {
...
export async function fetchFeuilleJournee(
  classeMatiereId: number,
  date?: string,
): Promise<FeuilleJournee> {
  const { data } = await http.get<ApiResponse<FeuilleJournee>>(
    `/ma-journee/${classeMatiereId}`,
    {
      params: date ? { date } : undefined,
    },
  );
...
export async function resoudreQr(token: string): Promise<ResolutionQr> {
  const { data } = await http.get<ApiResponse<ResolutionQr>>(
    `/ma-journee/qr/${token}`,
  );
  return data.data;
}
...
export async function fetchChampsPersonnalises(
  classeMatiereId: number,
...
export async function fetchEvaluations(
  ...
  const { data } = await http.get<ApiResponse<Evaluation[]>>(
    `/classe-matieres/${classeMatiereId}/evaluations`,
  );
  return data.data;
}
...
export async function creerEvaluation(
  ...
  const { data } = await http.post<ApiResponse<Evaluation>>(
    `/classe-matieres/${classeMatiereId}/evaluations`,
    payload,
  );
  return data.data;
...
export async function modifierEvaluation(
  ...
  const { data } = await http.put<ApiResponse<Evaluation>>(
    `/evaluations/${id}`,
    payload,
  );
  return data.data;
