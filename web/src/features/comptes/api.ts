...
export async function fetchComptesUtilisateurs(): Promise<CompteUtilisateur[]> {
  const { data } = await http.get<ApiResponse<CompteUtilisateur[]>>(
...
export async function fetchActiviteCompte(
  ...
  const { data } = await http.get<ApiResponse<EntreeActivite[]>>(
    `/comptes-utilisateurs/${compteId}/activite`,
    {
      params: { page },
    },
  );
...
export async function attribuerEcolesCompte(
  compteId: number,
  schoolIds: number[],
): Promise<void> {
  await http.put(`/comptes-utilisateurs/${compteId}/ecoles`, {
    school_ids: schoolIds,
  });
}

export async function basculerSuperAdmin(
  compteId: number,
  actif: boolean,
): Promise<void> {
  await http.put(`/comptes-utilisateurs/${compteId}/super-admin`, { actif });
}
...
export async function reinitialiserMotsDePasseJamaisConnectes(): Promise<{
  total: number;
}> {
...
export async function reinitialiserMotDePasseCompte(
  ...
  await http.post(
    `/comptes-utilisateurs/${compteId}/reinitialiser-mot-de-passe`,
    {
