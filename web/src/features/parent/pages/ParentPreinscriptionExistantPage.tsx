...
export function ParentPreinscriptionExistantPage() {
  ...
  const steps = [
    { id: 'identite', label: 'Identité', description: 'Nom, naissance, adresse' },
    { id: 'acte_sante', label: 'État civil & santé', description: "Acte de naissance, fiche sanitaire" },
    { id: 'tuteurs', label: 'Parents', description: 'Coordonnées des parents/tuteurs' },
    { id: 'paiement', label: 'Versement', description: 'Facultatif' },
  ...
  return (
    <div>
      ...
      <PageHeader titre="Préinscription" sousTitre={`Réviser la fiche de ${enfant.nom_complet} pour l'année à venir.`} />
          ...
          >
            {steps[currentStep]?.id === 'identite' && (
              <div className="space-y-4">
                ...
                <div className="rounded-xl bg-cream-100 p-3.5">
                  <p className="text-xs font-semibold uppercase tracking-wide text-navy-500">Classe actuelle</p>
                  <p className="mt-1 text-sm font-semibold text-navy-800">{enfant.classe?.nom || '—'}</p>
                  <p className="mt-1 text-xs text-navy-400">
                    La classe ne peut pas être modifiée depuis ce formulaire. En cas d'erreur, signalez-la ci-dessous.
                  </p>
                </div>
                <Textarea label="Note à l'attention de l'établissement (optionnel)" placeholder="Ex. mon enfant a été mal orienté, il devrait être en…" {...register('note_admin')} />
              </div>
            )}
            ...
            {steps[currentStep]?.id === 'acte_sante' && (
              <div className="space-y-4">
                ...
                <Textarea label="Situation sanitaire" placeholder="Toute information utile à l'infirmerie" {...register('situation_sanitaire')} />
              </div>
            )}
            ...
            {steps[currentStep]?.id === 'paiement' && (
              <div className="space-y-4">
                <p className="text-sm text-navy-500">
                  Facultatif — vous pouvez verser un acompte dès maintenant, ou laisser ce champ vide et régler plus tard à la caisse.
                </p>
                {finance && (
                  <dl className="grid grid-cols-3 gap-2 rounded-xl bg-cream-100 p-3 text-center">
                    {[
                      ...
                      ['Reste', francs(finance.reste_a_payer), 'text-red-500'],
                ...
                <Input label="Montant à verser (F CFA)" type="number" min={0} {...register('montant_verser')} />
                    ...
                    <table className="w-full min-w-[380px] text-xs">
                      <thead className="bg-cream-50 text-[10px] font-semibold uppercase tracking-wide text-navy-400">
                        <tr>
                          ...
                          <th className="px-2.5 py-2 text-right">Sera affecté</th>
                        </tr>
                      </thead>
                ...
                {montant > 0 && (
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <Select label="Mode de versement" {...register('mode_versement')}>
                      {MODES.map((m) => (
                    ...
                    <Input label="Référence (Mobile Money…)" placeholder="Facultatif" {...register('reference_externe')} />
                  </div>
                )}
                <p className="rounded-xl bg-cream-100 px-3.5 py-2.5 text-xs text-navy-500">
                  Ce versement n'est pas encaissé immédiatement : il attend la validation de l'établissement, qui délivrera alors le reçu.
                </p>
              </div>
            )}
            ...
            {steps[currentStep]?.id === 'confirmation' && (
              <div className="space-y-4">
                ...
                <p className="rounded-xl bg-green-50 px-3.5 py-2.5 text-sm text-green-800">
                  Votre demande sera transmise à l'établissement pour validation. Vous serez informé de la décision.
                </p>
              </div>
            )}
          </StepForm>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold uppercase tracking-wide text-navy-500">Parents / tuteurs</h3>
        <button
          type="button"
      ...
      {fields.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-navy-200 p-6 text-center text-sm text-navy-400">
          Aucun tuteur — ajoutez-en au moins un.
        </div>
      ) : (
          ...
          <Card key={field.id} className="bg-cream-50 p-4">
            <div className="grid grid-cols-1 gap-3">
              <Input
                ...
                {...register(`tuteurs.${index}.nom_complet` as const, { required: 'Requis.' })}
              />
              <Input label="Lien de parenté" placeholder="Père, mère, tuteur…" {...register(`tuteurs.${index}.lien_parente` as const)} />
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <Input label="Téléphone" type="tel" {...register(`tuteurs.${index}.telephone` as const)} />
                <Input label="E-mail" type="email" {...register(`tuteurs.${index}.email` as const)} />
              </div>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <Input label="Profession" {...register(`tuteurs.${index}.profession` as const)} />
                <Input label="Lieu de service" {...register(`tuteurs.${index}.lieu_service` as const)} />
              </div>
              <Input label="Adresse" {...register(`tuteurs.${index}.adresse` as const)} />
