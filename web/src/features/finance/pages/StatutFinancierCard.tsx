      ...
      {isLoading || !dossier ? (
        ...
        <>
          <div className="mb-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-navy-400">{t('eleves.financier.reste_scolarite_a_payer')}</p>
              <p className="text-lg font-bold tabular-nums text-navy-900">{francs(dossier.reste_scolarite_a_payer)}</p>
            </div>
            <div>
              ...
              <p className={`text-lg font-bold tabular-nums ${dossier.report_dette > 0 ? 'text-red-500' : 'text-navy-300'}`}>
                {francs(dossier.dette_anterieure_restante)}
              </p>
            </div>
            {dossier.avance > 0 && (
              <div>
