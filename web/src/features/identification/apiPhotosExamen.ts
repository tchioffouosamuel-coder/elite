...
export async function fetchClassesExamen(): Promise<ClasseExamen[]> {
  const { data } = await http.get<ApiResponse<ClasseExamen[]>>('/photos-examen/classes')
  return data.data
}

export async function fetchDossierExamen(classeId: number): Promise<DossierExamen> {
  const { data } = await http.get<ApiResponse<DossierExamen>>(`/photos-examen/classes/${classeId}`)
  return data.data
}
...
export async function telechargerArchiveExamen(
  ...
  const response = await http.get(`/photos-examen/classes/${classeId}/archive`, { responseType: 'blob' })

  const disposition = response.headers['content-disposition'] as string | undefined
  const nom = disposition?.match(/filename="?([^";]+)"?/)?.[1] ?? 'photos-examen.zip'
