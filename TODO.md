...
**Attributions nominatives et périmètre** — les colonnes de responsables ne sont plus de simples champs d'affichage : elles confèrent des prérogatives, et seulement sur les classes concernées. `App\Support\Attributions` catalogue les cinq responsabilités (professeur principal, surveillant général, censeur, conseiller d'orientation, chef de département) avec la colonne qui les porte, les fonctions éligibles et les privilèges conférés ; `App\Support\Perimetre` calcule l'étendue d'un compte (classes enseignées, classes et départements confiés) et tranche « ce privilège, sur cette classe ? » (`User::peutSurClasse()`). La distinction reprise de `_smapp` : **la fonction dit le métier, l'attribution dit l'étendue**, et les deux se cumulent — un enseignant désigné surveillant général d'une classe garde ses prérogatives d'enseignant là où il enseigne et gagne la discipline là où il surveille, sans pouvoir y saisir de notes ; un agent dont la fonction *est* surveillant général n'enseigne pas et ne tient que la discipline de ses classes assignées ; le censeur suit la même logique sur le versant pédagogique. Le middleware `permission` applique le périmètre dès qu'une route nomme une classe (et distingue « privilège manquant » de « classe hors périmètre ») ; les listes classes/élèves/sanctions s'y bornent via le scope `dansPerimetre`. La direction (`ecoles.manage`) et les fonctions transverses (économat, infirmerie) restent hors bornage. Côté clients : `GET /mes-attributions`, `UserResource.attributions`/`perimetre_borne`, écran « Mes attributions » (web et mobile), et filtre `attribution=` sur `/personnels` pour ne proposer que des candidats éligibles.
